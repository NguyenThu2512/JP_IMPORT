...
while True:
    try:
        ...
        if(true_logistic_data.shape[0]==0):
            ...
            true_logistic_data = vn_import_df[['product_id', 'vn_date_import', 'weight_rounded']].copy()
            print("Đã lấy được dữ liệu từ vn_import")
            #Lọc dữ liệu ở logistic_hub có product_id khớp với vn_import
            product_id_vn_import=true_logistic_data['product_id']
            logistic_hub_df=wks_logistic_hub.get_as_df(has_header=True)
            logistic_hub_df=logistic_hub_df.loc[(product_id_vn_import.isin(logistic_hub_df['product_id']))].reset_index()
        ...
        if (true_logistic_data.shape[0] == 0):
            ...
            product_id_vn_import = true_logistic_data['product_id']
            logistic_hub_df = wks_logistic_hub.get_as_df(has_header=True)
            logistic_hub_df = logistic_hub_df.loc[
                (product_id_vn_import.isin(logistic_hub_df['product_id']))].reset_index()
